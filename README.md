...
LeyoAI Cyber Model (Medium) 是一个中等规模的网络安全助手，基于 Qwen2.5-1.5B-Instruct 微调，提供专业的安全分析能力。
...
model = AutoModelForCausalLM.from_pretrained(
    "FFZwai/leyoai-cyber-medium",
    device_map="auto"
)
tokenizer = AutoTokenizer.from_pretrained("FFZwai/leyoai-cyber-medium")

# Inference
prompt = "设计一个安全的用户认证系统需要考虑哪些方面？"
...
LeyoAI Cyber Model (Medium) is a medium-sized cybersecurity assistant fine-tuned from Qwen2.5-1.5B-Instruct, providing professional security analysis capabilities.
...
# Inference
prompt = "What aspects should be considered when designing a secure user authentication system?"
inputs = tokenizer(prompt, return_tensors="pt").to(model.device)
outputs = model.generate(**inputs, max_length=100)
response = tokenizer.decode(outputs[0], skip_special_tokens=True)
print(response)
